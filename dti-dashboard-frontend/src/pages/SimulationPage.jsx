...
// ... (featuresConfig, defaultFeatures, glassmorphismStyle không đổi) ...
const featuresConfig = [
  { key: "HaTangSo", label: "Hạ tầng số", min: 0, max: 1, step: 0.01 },
  { key: "DanSo", label: "Dân số (nghìn)", min: 100, max: 10000, step: 100 },
  {
...
const getDtiResult = (score) => {
  ...
  if (score > 60) return { text: "Trung bình", color: "#fa8c16" };
  return { text: "Thấp", color: "#ff4d4f" };
};
...
const SimulationPage = () => {
  ...
  // ... (Tất cả state và hàm (handleInputChange, handlePredictClick, renderFeatureControl) giữ nguyên) ...
  const [features, setFeatures] = useState(defaultFeatures);
  const [prediction, setPrediction] = useState(null);
  const [isPredicting, setIsPredicting] = useState(false);
  const handleInputChange = (key, value) => {
  ...
  return (
    <div>
      {/* ... (Tiêu đề và Paragraph giữ nguyên) ... */}
      <Title
        level={2}
        style={{
          textAlign: "center",
          marginBottom: "16px",
          background: "linear-gradient(90deg, #00b96b, #00874e)",
          WebkitBackgroundClip: "text",
          WebkitTextFillColor: "transparent",
        }}
      >
      ...
      <Row gutter={[16, 16]}>
        ...
        {/* === CỘT OUTPUT (Đã thêm Card chú giải) === */}
        <Col xs={24} md={12}>
          {/* 1. CARD KẾT QUẢ (Không đổi) */}
          <Card
            title="Kết Quả Dự Đoán"
            bordered={false}
            headStyle={{ textAlign: "center" }}
          ...
          >
            ...
            >
              {/* ... (Code Spin, Progress, Title, Paragraph giữ nguyên) ... */}
              {isPredicting && <Spin tip="Đang gọi API mô hình..." />}
              {!isPredicting && prediction !== null && (
                <>
                  {(() => {
                    const dtiScore = prediction * 100;
                    const result = getDtiResult(dtiScore);
                    const percent = parseFloat(dtiScore.toFixed(1));
                    return (
                      <>
                        <Progress
                          type="circle"
          ...
          {/* 2. CARD CHÚ GIẢI MỚI (DÁN VÀO ĐÂY) */}
          <Card
            title="Chú giải Thang điểm Đánh giá"
            bordered={false}
            style={{ ...glassmorphismStyle(borderRadiusLG), marginTop: "16px" }}
            className="dashboard-card"
            headStyle={{ textAlign: "center" }}
          >
            <Space direction="vertical" style={{ width: "100%" }}>
              <Row>
                <Col span={12}>
                  <Tag color={getDtiResult(80).color}>Rất cao</Tag>
                </Col>
                <Col span={12}>
                  <Text
                    style={{
              ...
              <Row>
                <Col span={12}>
                  <Tag color={getDtiResult(72).color}>Cao</Tag>
                </Col>
                <Col span={12}>
                  <Text
                    style={{
              ...
              <Row>
                <Col span={12}>
                  <Tag color={getDtiResult(67).color}>Khá</Tag>
                </Col>
                <Col span={12}>
                  <Text
                    style={{
              ...
              <Row>
                <Col span={12}>
                  <Tag color={getDtiResult(62).color}>Trung bình</Tag>
                </Col>
                <Col span={12}>
                  <Text
                    style={{
              ...
              <Row>
                <Col span={12}>
                  <Tag color={getDtiResult(50).color}>Thấp</Tag>
                </Col>
                <Col span={12}>
                  <Text
                    style={{
